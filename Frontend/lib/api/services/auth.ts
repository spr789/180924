...
export class AuthService {
  ...
  async getProfile(): Promise<ApiResponse<UserProfile>> {
    ...
    return await this.client.get<ApiResponse<UserProfile>>(
  ...
  async updateProfile(
    ...
    return this.client.patch<ApiResponse<UserProfile>>(
      API_ENDPOINTS.PROFILE,
      data
    );
  }
  ...
  async changePassword(data: {
    ...
    return this.client.post<ApiResponse<void>>(
  ...
  async vendorLogin(credentials: LoginCredentials): Promise<
    ApiResponse<{
      refresh: string;
      access: string;
      user: User & { vendor_profile: VendorProfile };
    }>
  > {
    ...
    >(API_ENDPOINTS.VENDOR_LOGIN, credentials);
  ...
  async vendorRegister(data: RegisterData & { business_name: string }): Promise<
    ApiResponse<{
      refresh: string;
      access: string;
      user: User & { vendor_profile: VendorProfile };
  ...
  > {
    ...
    >(API_ENDPOINTS.VENDOR_REGISTER, data);
    console.log(
      'Vendor registration successful, setting token:',
      response.access
    );
    this.client.setToken({
      access: response.access,
  ...
  async getVendorProfile(): Promise<ApiResponse<VendorProfile>> {
    ...
    return this.client.get<ApiResponse<VendorProfile>>(
      API_ENDPOINTS.VENDOR_PROFILE
    );
  }
  ...
  async updateVendorProfile(
    ...
    return this.client.patch<ApiResponse<VendorProfile>>(
      API_ENDPOINTS.VENDOR_PROFILE,
      data
    );
  }
