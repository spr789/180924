...
import { Vendor, PaginatedResponse, Product } from '../types';
...
export class VendorService {
  async getVendors(params?: {
    ...
    try {
      const response = await axiosInstance.get<PaginatedResponse<Vendor>>('/vendors', { params });
  ...
  async unfollowVendor(vendorId: string) {
    try {
      const response = await axiosInstance.delete(`/vendors/${vendorId}/follow`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async contactVendor(vendorId: string, data: {
    subject: string;
    message: string;
  }) {
    try {
      const response = await axiosInstance.post(`/vendors/${vendorId}/contact`, data);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
